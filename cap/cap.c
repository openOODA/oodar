/* Named oodar_cap_* seccomp-bpf: unauthorized socket/connect/bind, fork/execve
...
#if defined(__linux__) && defined(PR_SET_SECCOMP)
  {
    int proc_ok = (allowed_caps_mask & (OODAR_CAP_PROCESS | OODAR_CAP_SYS)) != 0;
    int fsw_ok = (allowed_caps_mask & (OODAR_CAP_FSWRITE | OODAR_CAP_FS)) != 0;
    uint32_t net_act = (allowed_caps_mask & OODAR_CAP_NET) ? SECCOMP_RET_ALLOW : SECCOMP_RET_KILL_PROCESS;
