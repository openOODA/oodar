/* gpu/gpu_launch.c — kernel dispatch entry point.
 * The HIP backend kernels (vec_add / sgemm / rmsnorm / attention /
 * reduce_sum / rope / stencil_3d) live in gpu_hip_dispatch.c. Kernel
 * classification (name + structural marker detection) lives in
 * gpu_classify.c, which is included before this TU in the umbrella
 * and provides the OoGpuKernelKind enum. This file owns:
 *   - the forward decls to gpu_hip_* dispatch
