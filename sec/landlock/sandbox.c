...
/* v2.2.0: C-ABI "sandbox" entry points. Previously each of these called
 * oo_cap_grant_sys() internally to manufacture its own capability token,
 * which bypassed the cap system entirely (a covert backdoor — any C
 * caller could apply a sandbox matrix without holding SysCap). Now the
 * caller MUST supply the sys_cap as the first argument; the function
 * validates it via oo_cap_require_sys(), which fails closed (exits) on
 * a missing or forged token. */
