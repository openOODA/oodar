...
/* Plan v29: when the Landlock update bypass fires, also skip the
 * seccomp filter and rlimits so `ooda update`'s child bash can run
 * curl (network). User has typed `ooda update` explicitly. */
OoResS sand_apply_linux_landlock_seccomp(long long sys_cap, const oo_sandbox_config_t *config, int nread, int nwrite) {
  ...
  if (config->read_dirs_colon.len > 0 || config->write_dirs_colon.len > 0 || (nread == 0 && nwrite == 0)) {
    OoResS ll_res = oo_landlock_restrict(sys_cap, config->read_dirs_colon, config->write_dirs_colon);
    if (!ll_res.ok) return ll_res;
    if (ll_res.val.len == 24 && memcmp(ll_res.val.data, "OK_LANDLOCK_UPDATE_BYPASS", 24) == 0) {
      landlock_bypassed = 1;
    }
  }
