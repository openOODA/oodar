...
static OoResS darwin_seatbelt_apply(const oo_sandbox_config_t *cfg) {
  ...
  if ((cfg->allowed_caps_mask & (CHS_CAP_FSWRITE | CHS_CAP_FS)) &&
  ...
  if (cfg->allowed_caps_mask & (CHS_CAP_NET | CHS_CAP_HTTP | CHS_CAP_TCP | CHS_CAP_UDP | CHS_CAP_BIND)) {
    off += (size_t)snprintf(profile + off, sizeof(profile) - off,
      "(allow network*)\n(allow system-socket)\n");
  }

  if (cfg->allowed_caps_mask & (CHS_CAP_PROCESS | CHS_CAP_SYS)) {
    off += (size_t)snprintf(profile + off, sizeof(profile) - off,
      "(allow process-exec)\n(allow process-fork)\n");
  }
...
OoResS oo_sandbox_apply_matrix(long long sys_cap, const oo_sandbox_config_t *config) {
  ...
  if (backend == OO_SANDBOX_BACKEND_LINUX_LANDLOCK_SECCOMP) {
    ...
    if (chs_cap_apply_seccomp_filter(config->allowed_caps_mask) < 0) {
  ...
  g_sand_status.net_restricted = (config->allowed_caps_mask & (CHS_CAP_NET | CHS_CAP_HTTP | CHS_CAP_TCP | CHS_CAP_UDP | CHS_CAP_BIND)) ? 0 : 1;
  g_sand_status.proc_restricted = (config->allowed_caps_mask & (CHS_CAP_PROCESS | CHS_CAP_SYS)) ? 0 : 1;
